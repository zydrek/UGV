#fase 3.1.3  - centralização do robo pelo id==0.  
#usando dois arucos nas laterais para centralização. Resolver o problema do 
FOV de 3.1.2 
#trocamos a visualização para os dois +proximos e os 2+longe para criação de 
cenários.  
#upgrade na inercia e controlo P 
...
# VARIAVEL GLOBAL DE INÉRCIA DECLARADA AQUI 
c1 = 0  
...
======================================================== 
            ...
            if par_valido is not None: 
                ...
                print(f"Par Encontrado! (Esq: {int(esq['area'])} | Dir: {int(dir['area'])}) -> 
Erro: {erro_x}") 
                base.base_json_ctrl({"T":13, "X":0.3, "Z": -kp * erro_x}) 
...
{int(unico['area'])}") 
                    ...
                    if unico['area'] > LIMIAR_PERIGO: 
                        ...
                        if unico['x'] > 320: 
                            base.base_json_ctrl({"T":13, "X":0.3, "Z": -0.15}) 
                        else: 
                            base.base_json_ctrl({"T":13, "X":0.3, "Z": 0.15}) 
